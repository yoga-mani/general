...
class fixedwidth_data_process:
    ...
    def metadata_process(self, metadata):
    ...
    def set_hadoop_configuration(self):
        self.sc._jsc.hadoopConfiguration().set("mapreduce.fileoutputcommitter.marksuccessfuljobs", "false")
    ...
    def convert_fixed_width_row(self,row, file_name, key_columns, table_name):
        transformed_row = []
        dataops_etl_dq_er = []
        key_column_values = []

        for start, end, datatype, column_name in zip(self.abs_offsets, self.abs_offsets[1:], self.datatypes, self.cols):
    ...
    def get_column_metadata(self,metadata_file_path):
    ...
    def process_fixed_width_file(self, df, metadata, valid_rec_length, key_column_names, table_name):
        ...
        transformed_data = good_data_df.rdd.map(
            lambda row: self.convert_fixed_width_row(
                row['value'],
    ...
    def create_dataframe_with_schema(self, processed_data, metadata):
        ...
        schema = StructType([
                                StructField(name, self.get_spark_datatype(datatype), True)
    ...
    def marker_file_creation(self,output_file_path, table_name):
        ...
        self.s3.put_object(Bucket=bucket, Key=marker_file_s3_path, Body=b'')

        logger.info(f"Marker file '{marker_file_name}' written to: s3://{bucket}/{marker_file_s3_path}")


    def list_s3_files(self,raw_s3_bucket_name, raw_s3_metadata_prefix, execution_dag_date, file_prefix):
        logger.info("latest metadata file to be found from the mata s3 uri ")
        file_prefix = str(file_prefix).replace(".", "")
        s3_metadata_objs = self.s3.list_objects_v2(Bucket=raw_s3_bucket_name, Prefix=raw_s3_metadata_prefix)
    ...
    def list_s3_data_files_with_prefix(self,bucket_name, prefix, file_prefix):
        s3_data_objs = self.s3.list_objects_v2(Bucket=bucket_name, Prefix=prefix)
    ...
    def rename_csv_files_in_s3(self,s3_uri, base_filename, destination_prefix):
        ...
        response = self.s3.list_objects_v2(Bucket=bucket_name, Prefix=source_prefix)
        filename_extn = 'CSV'
        if 'Contents' in response:
            for obj in response['Contents']:
                ...
                self.s3.copy_object(Bucket=bucket_name, CopySource={'Bucket': bucket_name, 'Key': source_key}, Key=new_key)
                self.s3.delete_object(Bucket=bucket_name, Key=obj['Key'])
                logger.info(f"File renaming completed in the new path - {new_key}")

    def s3_path_exists(self,s3_path):
    ...
    def process_source_marker_file(self,marker_file_path,metric_name):
        ...
        rdd_with_index = self.spark.sparkContext.textFile(marker_file_path).zipWithIndex()
    ...
    def generate_summary_file(self,processed_file_name, order_date,dq_issue_distinct_count_per_file_df,
        ...
        # source_weekly_counts = self.process_source_marker_file(periodic_marker_s3_key,"source_distinct_count")
        ...
        file_names_df = self.spark.createDataFrame([(name,) for name in file_names_in_decrypt], ["file_name_s3"])
        ...
        summary_df = self.spark.createDataFrame(summary_data, schema)
        ...
        summary_header_df = self.spark.createDataFrame([header_row])
    ...
    def main(self,processed_file_location, raw_s3_data_prefix, raw_s3_metadata_prefix, raw_s3_bucket_name,
        ...
        s3_data_files, s3_data_file_names = self.list_s3_data_files_with_prefix(raw_s3_bucket_name, raw_s3_data_prefix,
                                                                                file_prefix)
        # Obtain weekly marker files s3 keys
        # s3_bucket_ws, periodic_marker_s3_prefix = self.split_s3_uri(landing_periodic_marker_s3_uri)
        # file_prefix_name = file_prefix.replace(".", "")
        # _, periodic_marker_s3_file_name_list = self.list_s3_data_files_with_prefix(raw_s3_bucket_name, periodic_marker_s3_prefix, file_prefix_name)
        ...
        # Fetch metadata information and initialize
        metadata_file_path = self.list_s3_files(raw_s3_bucket_name, raw_s3_metadata_prefix, execution_dag_date, file_prefix)
        logger.info(f"{metadata_file_path} --> metadata_dict ")
        metadata_dict, key_column_names, valid_rec_length = self.get_column_metadata(metadata_file_path)
        self.metadata_process(metadata_dict)
        ...
        df = self.spark.read.text(f"{s3_uri}")
        ...
        data_df = self.create_dataframe_with_schema(good_data_df, metadata_dict)
        rejected_data_with_dq_df = self.create_rejected_data_df(rejected_data_with_dq_issues, self.exception_schema_fields)
        ...
        header_row = Row(**header_dict)
        # Create a DataFrame using a list containing the Row object
        dq_header_df = self.spark.createDataFrame([header_row])
        ...
        if dq_record_count > 0:
            ...
            self.rename_csv_files_in_s3(rejected_dq_header_part_file_s3_uri,"header", rejected_dq_header_file_s3_prefix)
...
if __name__ == '__main__':
    ...
    fixedwidth_ingestion_obj = fixedwidth_data_process()
    fixedwidth_ingestion_obj.main(processed_file_location, raw_s3_data_prefix, raw_s3_metadata_prefix, raw_s3_bucket_name,
                                  processed_file_name,
